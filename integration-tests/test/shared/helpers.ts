...
export const deployLoadTestContracts = async (
  ...
  const Factory__L2DepositTracker = await ethers.getContractFactory('L2DepositTracker', l2Wallet)
  const Factory__L1DepositInitiator = await ethers.getContractFactory('L1DepositInitiator', l1Wallet)
  const Factory__L2TxStorage = await ethers.getContractFactory('L2TxStorage', l1Wallet)
  if (!L2_DEPOSIT_TRACKER_ADDRESS) {
    l2DepositTracker = await Factory__L2DepositTracker.deploy()
    logger.info(`l2DepositTracker address on L2: ${l2DepositTracker.address}`)
  } else {
    l2DepositTracker = Factory__L2DepositTracker.attach(L2_DEPOSIT_TRACKER_ADDRESS)
    logger.info(
      `connecting to existing l2DepositTracker at ${L2_DEPOSIT_TRACKER_ADDRESS}`
    )
  }
  if (!L1_DEPOSIT_INITIATOR_ADDRESS) {
    l1DepositInitiator = await Factory__L1DepositInitiator.deploy()
    logger.info(
      `l1DepositInitiator address on L1: ${l1DepositInitiator.address}`
    )
  } else {
    l1DepositInitiator = Factory__L1DepositInitiator.attach(L1_DEPOSIT_INITIATOR_ADDRESS)
    logger.info(
      `connecting to existing l1DepositInitiator at ${L1_DEPOSIT_INITIATOR_ADDRESS}`
    )
  }
  if (!L2_TX_STORAGE_ADDRESS) {
    l2TxStorage = await Factory__L2TxStorage.deploy()
    logger.info(`l2TxStorage address on L2: ${l2TxStorage.address}`)
  } else {
    l2TxStorage = Factory__L2TxStorage.attach(L2_TX_STORAGE_ADDRESS)
    logger.info(
